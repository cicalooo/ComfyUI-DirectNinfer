...
The following is the reference environment used for the published evaluation:
...
The `.ninfer` file is a model artifact, not a GPU executable. The NInfer
runtime must contain kernels for the target CUDA architecture, so select a
matching build or compile one for the card you intend to use. The reference
requirements above do not describe every supported GPU.

| GPU | Runtime direction | Starting guidance |
|---|---|---|
| RTX 3090 / 30-series (`sm_86`) | [NInfer-3090 Windows release](https://github.com/Don-Chad/ninfer-3090/releases/tag/v0.6.0-rtx3090) | Use the 24 GB profile: 4K context/KV, INT8 KV, MTP3, and CUDA Graphs disabled initially. Do not assume 16 GB cards fit. |
| RTX 4090 / 40-series (`sm_89`) | A Qwen3.8-compatible `sm_89` build or port | Do not use the RTX 3090 `sm_86` binary. Validate the exact card and available VRAM. |
| RTX 5090 (`sm_120a`) | Upstream Linux build or a native Windows port | This is the reference path; use CUDA 13.1 or newer and leave VRAM headroom. |
| Other NVIDIA CUDA GPUs | A runtime explicitly listing the card's architecture | Do not infer support from the product series alone; start with a small context/KV allocation. |
| AMD, Intel, Apple, or CPU-only systems | None in this CUDA/NInfer path | Unsupported by this artifact/runtime combination. |

The node launches one NInfer process on one CUDA device; it does not combine
VRAM across multiple GPUs. For node-specific installation and tuning examples,
see the [higher-tier GPU guide](../GPU_INSTALLATION_GUIDE.md). Always run the
selected executable with `--help` and make one test request before using it in
ComfyUI.
...
- The artifact is accepted only by NInfer revision `5232055` or later and the matching registered
  target.
- The reference NInfer runtime executes on one RTX 5090 and one CUDA device,
  with a startup-fixed capacity of 1–8 active requests per Engine.
